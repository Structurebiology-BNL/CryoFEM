...
class ResidualDenseBlock_5C(nn.Module):
    def __init__(
        ...
        # gc: growth channel
        self.conv1 = conv(
            nc, gc, kernel_size, stride, padding, bias, mode, negative_slope
        )
        self.conv2 = conv(
            nc + gc, gc, kernel_size, stride, padding, bias, mode, negative_slope
        )
        self.conv3 = conv(
            nc + 2 * gc, gc, kernel_size, stride, padding, bias, mode, negative_slope
        )
        self.conv4 = conv(
            nc + 3 * gc, gc, kernel_size, stride, padding, bias, mode, negative_slope
        )
        self.conv5 = conv(
            nc + 4 * gc,
            nc,
...
class NonLocalBlock2D(nn.Module):
    def __init__(
        ...
        self.W = conv(
            inter_nc, nc, kernel_size, stride, padding, bias, mode="C" + act_mode
        )
        ...
        if downsample:
            ...
            self.phi = downsample_block(
                nc, inter_nc, kernel_size, stride, padding, bias, mode="2"
            )
            self.g = downsample_block(
                nc, inter_nc, kernel_size, stride, padding, bias, mode="2"
            )
        else:
